<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap justify-between items-center gap-2">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('OT Requests') }}</h2>
            <div class="flex items-center gap-2">
                ...
                <x-primary-button onclick="openOtCreate()" type="button">{{ __('Create OT Request') }}</x-primary-button>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Created Date') }}</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-block text-xs px-2 py-1 rounded
                                        ...
                                        {{ ['pending' => __('Pending'), 'approved' => __('Approved'), 'rejected' => __('Rejected')][$ot->status] ?? ucfirst($ot->status) }}
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    @if($ot->approver)
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <button onclick="openOtModal({{ $ot->id }})" title="{{ __('View') }}"
                                        ...
                                        @if(!in_array($ot->status, ['approved', 'rejected']))
                                            ...
                                            @if($canEdit)
                                                <button onclick="openOtModal({{ $ot->id }})" title="{{ __('Edit') }}"
                        ...
                        @empty
                            <tr><td colspan="12" class="px-6 py-10 text-center text-gray-400">{{ __('No OT requests found.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
