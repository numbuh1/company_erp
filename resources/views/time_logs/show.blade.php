<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('Time Log') }}</h2>
            <div class="flex gap-2">
                @if($canEdit)
                    <a href="{{ route('time-logs.edit', $timeLog) }}"><x-secondary-button>{{ __('Edit') }}</x-secondary-button></a>
                @endif
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-700 dark:text-gray-300">
                        @if($timeLog->task)
                            ...
                            @if($timeLog->project)
                                <span class="text-gray-400 mx-1">{{ __('in') }}</span>
                        ...
                        @else
                            <span class="text-gray-400">{{ __('Other (no project or task)') }}</span>
                        @endif
                    </p>
                </div>
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                        <form method="POST" action="{{ route('time-logs.destroy', $timeLog) }}">
                            ...
                            <button type="submit" onclick="return confirm('Delete this time log?')"
                                class="text-sm text-red-600 hover:underline">{{ __('Delete this log') }}</button>
                        </form>
                    </div>
                @endif
            </div>
