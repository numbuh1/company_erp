            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Job Position') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Search Period') }}</th>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                <td class="px-4 py-4">
                                    ...
                                    <span class="ml-1 text-xs font-medium px-1.5 py-0.5 rounded {{ $posStatusColor($position->status) }}">
                                        {{ ['upcoming' => __('Upcoming'), 'in_progress' => __('Recruiting'), 'done' => __('Closed')][$position->status] ?? ucfirst(str_replace('_', ' ', $position->status)) }}
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-4 text-sm">
                                    <div class="flex flex-wrap gap-1">
                                        @if($position->applicants_count === 0)
                                            <span class="text-gray-400">—</span>
                                        @else
                                            <p class="text-xs font-medium px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400" title="{{ __('Total') }}">
                                                {{ __('Total') }}: {{ $position->applicants_count }}
                                            </p>
                                            @if($position->potential_count)
                                                @php $s = 'Tiềm năng'; @endphp
                                                <br><p class="text-xs font-medium px-1.5 py-0.5 rounded {{ \App\Models\RecruitmentApplicant::statusColor($s) }}" title="{{ \App\Models\RecruitmentApplicant::statusLabel($s) }}">{{ \App\Models\RecruitmentApplicant::statusLabel($s) }}: {{ $position->potential_count }}</p>
                                            @endif
                                            ...
                                            @endif
                                            @if($position->offer_consider_count)
                                                @php $s = 'Cân nhắc offer'; @endphp
                                                <br><p class="text-xs font-medium px-1.5 py-0.5 rounded {{ \App\Models\RecruitmentApplicant::statusColor($s) }}" title="{{ \App\Models\RecruitmentApplicant::statusLabel($s) }}">{{ \App\Models\RecruitmentApplicant::statusLabel($s) }}: {{ $position->offer_consider_count }}</p>
                                            @endif
                                            @if($position->offered_count)
                                                @php $s = 'Đã gửi offer'; @endphp
                                ...
                                <td class="px-4 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        @can('edit recruitment')
                                            ...
                                            <form method="POST" action="{{ route('recruitment.destroy', $position) }}" onsubmit="return confirm('{{ addslashes(__('Delete this position?')) }}')">
                                                @csrf @method('DELETE')
