                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                ...
                                <td class="px-4 py-3">
                                    @if($holiday->repeats_annually)
                                        <span class="text-xs font-medium px-2 py-0.5 rounded bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300">{{ __('Annual') }}</span>
                                    @else
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.public-holidays.destroy', $holiday) }}" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" title="{{ __('Delete') }}" onclick="return confirm('Delete this holiday?')"
