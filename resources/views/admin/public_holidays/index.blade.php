<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('Public Holidays') }}</h2>
            <a href="{{ route('admin.public-holidays.create') }}"><x-primary-button>+ {{ __('Add Holiday') }}</x-primary-button></a>
        </div>
    </x-slot>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Start Date') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('End Date') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Repeats') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                ...
                                <td class="px-4 py-3">
                                    @if($holiday->repeats_annually)
                                        <span class="text-xs font-medium px-2 py-0.5 rounded bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-300">{{ __('Annually') }}</span>
                                    @else
                                        <span class="text-xs text-gray-400">{{ __('Once') }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.public-holidays.edit', $holiday) }}" title="{{ __('Edit') }}"
                                        ...
                                        <form method="POST" action="{{ route('admin.public-holidays.destroy', $holiday) }}" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" title="{{ __('Delete') }}" onclick="return confirm('{{ addslashes(__('Delete this holiday?')) }}')"
                        ...
                        @empty
                            <tr><td colspan="5" class="px-6 py-8 text-center text-gray-400">{{ __('No public holidays configured.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
