<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">{{ __('Teams') }}</h2>
            @can('edit teams')
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Teams') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Members') }}</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Team Lead') }}</th>
                                    ...
                                    <div class="inline-flex items-center gap-1">
                                        ...
                                        @can('delete teams')
                                            <form method="POST" action="{{ route('teams.destroy', $team) }}" class="inline">
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                        onclick="return confirm('{{ addslashes(__('Delete this team?')) }}')"
                                                        title="{{ __('Delete') }}"
